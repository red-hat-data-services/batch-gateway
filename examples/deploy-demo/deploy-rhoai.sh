...
deploy_llm_inference_service() {
    ...
    # TODO(RHOAIENG-78216): remove epp template override once upstream fixes --model-server-metrics-scheme flag
    local scheduler_config_yaml
    if [ "${ENABLE_FLOW_CONTROL}" = "true" ]; then
        log "Flow control enabled: EPP will use EndpointPickerConfig with flowControl feature gate"
        scheduler_config_yaml=$(cat <<'EOF'
      config:
        inline:
  ...
  router:
    ...
    gateway:
      refs:
        - name: ${GATEWAY_NAME}
          namespace: ${GATEWAY_NAMESPACE}
    # if there is no gateway.refs defined: KServe uses the default gateway configured in
...
data:
  prometheus.yml: |
    ...
    - job_name: 'epp'
      ...
      static_configs:
      - targets: ['${isvc_name}-epp-service.${LLM_NAMESPACE}.svc.cluster.local:9090']
    - job_name: 'vllm'
      scheme: https
      tls_config:
...
verify_dispatcher_config() {
    ...
    step "Checking batch-processor configmap..."
    local config_yaml
    ...
    local dispatch_mode_config
    dispatch_mode_config=$(echo "${config_yaml}" | yq '.dispatch_mode // "sync"')
    if [ "${dispatch_mode_config}" = "async" ]; then
...
deploy_batch_gateway_rhoai() {
    ...
    if [ "${ENABLE_DISPATCHER}" = "true" ]; then
        local pool_name
        pool_name=$(discover_inference_pool)
        install_prometheus_for_dispatcher "${pool_name}" "${ISVC_NAME}"
...
EOF
    ...
    if [ "${ENABLE_DISPATCHER}" = "true" ]; then
        local pool_name
        pool_name=$(discover_inference_pool)
        verify_dispatcher_runtime "${BATCH_INSTANCE_NAME}-async-processor" "${pool_name}"
    fi
